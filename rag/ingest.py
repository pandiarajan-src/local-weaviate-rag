...
from .utils import env, logger, chunk_text, backoff_retry, get_weaviate_client
...
def main():
    ...
    parser.add_argument("input", help="Either the raw text to ingest OR a path to a .txt/.md file")
    parser.add_argument("--source", default="", help="Source label (filename, URL, note)")
    ...
    if os.path.isfile(args.input):
        ...
        try:
            ...
            if file_size > 50 * 1024 * 1024:  # 50MB
                logger.error(f"File too large: {file_size / 1024 / 1024:.1f}MB (max 50MB)")
                sys.exit(2)
        except OSError as e:
        ...
        try:
            ...
            try:
                with open(resolved_path, "r", encoding="latin-1") as f:
                    text = f.read()
                logger.warning(f"File decoded with latin-1 encoding instead of UTF-8")
            except Exception:
                logger.error(f"Failed to decode file '{args.input}' with UTF-8 or latin-1")
                sys.exit(2)
    ...
    # chunk
    chunks = chunk_text(text, model=embed_model, chunk_tokens=chunk_tokens, overlap_tokens=chunk_overlap)
    logger.info(f"Chunked into {len(chunks)} segments (≈{chunk_tokens} tokens target, {chunk_overlap} overlap).")
