        ...
        if len(sentences) == 1:
            ...
            for i, ch in enumerate(para):
                ...
                if ch in ".!?" and i < len(para) - 1:
                    ...
                    if para[i + 1].isspace() and i + 2 < len(para) and para[i + 2].isupper():
...
def backoff_retry(fn, *, retries: int = 5, base_delay: float = 0.5, factor: float = 2.0, exceptions=(Exception,)):
    """
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                if attempt == retries:
                    ...
                    raise
                logger.warning(f"Error: {e} — retrying in {delay:.1f}s (attempt {attempt}/{retries})")
                time.sleep(delay)
                delay *= factor
    return wrapper
