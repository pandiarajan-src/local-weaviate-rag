...
class RAGFunctionalityTest:
    ...
    def check_env_file(self) -> bool:
        ...
        try:
            ...
            self.log_test_result(
                "Environment File Check", False, f"Error reading .env: {e}"
            )
            return False
        ...
        self.log_test_result(
            "Environment File Check", True, "All required variables present"
        )
        return True
    ...
    def check_docker_running(self) -> bool:
        ...
        if success:
            ...
            self.log_test_result(
                "Docker Status Check", False, f"Docker not running: {stderr}"
            )
            return False
    ...
    def check_weaviate_running(self) -> bool:
        ...
        if success and "Up" in stdout:
            self.log_test_result(
                "Weaviate Container Check", True, "Weaviate is running"
            )
            return True
        else:
            self.log_test_result(
                "Weaviate Container Check", False, "Weaviate is not running"
            )
            return False
    ...
    def start_weaviate(self) -> bool:
        ...
        if success:
            ...
            if self.check_weaviate_running():
                self.log_test_result(
                    "Weaviate Startup", True, "Weaviate started successfully"
                )
                return True
    ...
    def create_test_files(self) -> list[str]:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            f.write(
                "This is a simple text file for testing. It contains basic information about testing procedures."
            )
...
"""
    ...
    "logging": {
        ...
        self.log_test_result(
            "Test File Creation", True, f"Created {len(test_files)} test files"
        )
        return test_files
    ...
    def test_ingestion(self, test_files: list[str]) -> bool:
        ...
        for i, file_path in enumerate(test_files):
            ...
            if success and "Ingested" in output_text and "chunks into" in output_text:
                self.log_test_result(
                    test_name, True, f"Successfully ingested {file_type} file"
                )
                ...
                self.log_test_result(
                    test_name,
                    False,
                    f"Failed - Command succeeded: {success}, Output: {output_text[:200]}...",
                )
        ...
        if success and "Ingested" in output_text and "chunks into" in output_text:
            self.log_test_result(
                "Direct Text Ingestion (Short)",
                True,
                "Successfully ingested short text",
            )
            ingestion_tests.append(True)
        else:
            self.log_test_result(
        ...
        cmd = (
            f'uv run python -m rag.ingest "{long_text.strip()}" --source "NLP Overview"'
        )
        success, stdout, stderr = self.run_command(cmd)
        ...
        if success and "Ingested" in output_text and "chunks into" in output_text:
            self.log_test_result(
                "Direct Text Ingestion (Long)", True, "Successfully ingested long text"
            )
            ingestion_tests.append(True)
        else:
            self.log_test_result(
                "Direct Text Ingestion (Long)",
                False,
                f"Failed - Command succeeded: {success}, Output: {output_text[:200]}...",
            )
            ingestion_tests.append(False)
        ...
        if (
            success
            and "already exists and is functional" in output_text
            and "Ingested" in output_text
        ):
            self.log_test_result(
                "Collection Persistence", True, "Collection reused without data loss"
            )
            ingestion_tests.append(True)
        else:
            self.log_test_result(
    ...
    def test_querying(self) -> bool:
        ...
        test_queries = [
            ...
            (
                "What database configuration is mentioned?",
                "database",
                "Configuration query",
            ),
        ...
        for query, expected_keyword, test_type in test_queries:
            ...
            if success:
                ...
                answer_section = (
                    stdout.split("GENERATED ANSWER:")[-1] if has_answer else ""
                )

                if has_results and has_answer and len(answer_section.strip()) > 10:
                    self.log_test_result(
                        f"Query Test: {test_type}",
                        True,
                        f"Got relevant results for: {query}",
                    )
                    ...
                    self.log_test_result(
                        f"Query Test: {test_type}",
                        False,
                        f"No meaningful results for: {query}",
                    )
                    query_tests.append(False)
            else:
                self.log_test_result(
                    f"Query Test: {test_type}", False, f"Query failed: {stderr}"
                )
                query_tests.append(False)
        ...
        edge_case_queries = [
            ...
            (
                "What is the meaning of life, universe and everything according to Douglas Adams?",
        ...
        for query, test_name in edge_case_queries:
            ...
            # Non-existent content should still return some response without crashing
                ...
                self.log_test_result(
                    test_name, True, "Edge case handled without crashing"
                )
                query_tests.append(True)
            else:
                self.log_test_result(
                    test_name, False, f"Edge case caused crash: {stderr}"
                )
                query_tests.append(False)
    ...
    def test_output_format(self) -> bool:
        ...
        self.log_test_result(
            "Output Format Test", True, "All required output sections present"
        )
        return True
    ...
    def test_schema_persistence(self) -> bool:
        ...
        if not success1:
            self.log_test_result(
                "Schema Persistence Test", False, f"Initial ingestion failed: {stderr1}"
            )
            return False

        # Then, query for it to ensure it was stored
        cmd = 'uv run python -m rag.query "unique test document"'
        ...
        if not success2:
            self.log_test_result(
                ...
                False,
                f"Query after ingestion failed: {stderr2}",
            )
            return False
        ...
        if unique_content.split()[0] in stdout2:  # Check for part of unique content
            self.log_test_result(
                "Schema Persistence Test",
                True,
                "Data persisted correctly between operations",
            )
            return True
        else:
            self.log_test_result(
                "Schema Persistence Test", False, "Data not found after ingestion"
            )
            return False
    ...
    def print_summary(self):
        ...
        for result in self.test_results:
            category = (
                result["test"].split(" ")[0] if " " in result["test"] else "Other"
            )
        ...
        for category, tests in categories.items():
            ...
            cat_total = len(tests)
            status = (
                "✅" if cat_passed == cat_total else "⚠️" if cat_passed > 0 else "❌"
            )
        ...
        try:
            ...
            if not self.check_weaviate_running():
                ...
                if not self.start_weaviate():
                    logger.error(
                        "❌ Failed to start Weaviate. Please check Docker logs."
                    )
                    return False
